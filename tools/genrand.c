...
static long nQ = 127773;        /* the quotient INT_MAX / Mult */
static long nR = 2836;          /* the remainder INT_MAX % Mult */
...
next_random_float (int stream)
{
   ...
   return ((double) res / (double) INT_MAX);
}
...
genrand_integer (int *dest, int dist, int min, int max, int mean, int stream)
{
   ...
   switch (dist)
     ...
     case DIST_EXPONENTIAL:
        for (i = 0; i < 12; i++)
           fres += (double) (next_random (stream) / INT_MAX) - 0.5;
        ...
        for (i = 0; i < 12; i++)
          {
             fres /= 2.0;
             fres +=
                (double) ((double) next_random (stream) / (double) INT_MAX) -
                0.5;
          }
        res.number =
...
/* The book says that this will work if INT_MAX for the type you choose    */
